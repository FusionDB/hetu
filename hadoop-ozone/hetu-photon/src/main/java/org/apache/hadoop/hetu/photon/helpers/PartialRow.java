...
public class PartialRow {
    ...
    public void addBoolean(int columnIndex, boolean val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.BOOL);
    ...
    public void addByte(int columnIndex, byte val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.INT8);
    ...
    public void addShort(int columnIndex, short val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.INT16);
    ...
    public void addInt(int columnIndex, int val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.INT32);
    ...
    public void addLong(int columnIndex, long val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.INT64, ColumnType.UNIXTIME_MICROS);
    ...
    public void addFloat(int columnIndex, float val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.FLOAT);
    ...
    public void addDouble(int columnIndex, double val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.DOUBLE);
    ...
    public BigDecimal getDecimal(int columnIndex) {
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.DECIMAL);
        checkColumnExists(schema.getColumnByIndex(columnIndex));
        checkValue(columnIndex);
        ColumnSchema column = schema.getColumnByIndex(columnIndex);
        ColumnTypeAttributes typeAttributes = column.getTypeAttributes();
    ...
    public Timestamp getTimestamp(int columnIndex) {
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.UNIXTIME_MICROS);
        checkColumnExists(schema.getColumnByIndex(columnIndex));
        checkValue(columnIndex);
    ...
    public void addDate(int columnIndex, Date val) {
        checkNotFrozen();
        checkColumn(schema.getColumnByIndex(columnIndex), ColumnType.DATE);
    ...
    public void addVarchar(int columnIndex, String val) {
        ColumnSchema column = schema.getColumnByIndex(columnIndex);
        checkColumn(column, ColumnType.VARCHAR);
        checkNotFrozen();
    ...
    private void setNull(ColumnSchema column) {
        ...
        if (!column.isNullable()) {
            throw new IllegalArgumentException(column.getColumnName() + " cannot be set to null");
        }
        ...
        try {
            ...
            switch (col.getColumnType()) {
                ...
                case DECIMAL:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Unsupported column type: " + col.getColumnType());
            }
        } catch (ClassCastException e) {
            throw new IllegalArgumentException(
                    "Value type does not match column type " + col.getColumnType() +
     ...
     * and your existing type handling logic is based on Java types.
     *
     * The Object type is based on the column's {@link ColumnType}:
     *  Type.BOOL -> java.lang.Boolean
     *  Type.INT8 -> java.lang.Byte
     *  Type.INT16 -> java.lang.Short
     *  Type.INT32 -> java.lang.Integer
    ...
    public Object getObject(int columnIndex) {
        ...
        ColumnType type = schema.getColumnByIndex(columnIndex).getColumnType();
        switch (type) {
     ...
     */
    private void checkColumn(ColumnSchema column, ColumnType... types) {
        checkColumnExists(column);
        ...
        throw new IllegalArgumentException(String.format("%s isn't %s, it's %s", column.getColumnName(),
                Arrays.toString(types), column.getColumnType().getName()));
    }
    ...
    public String toString() {
        ...
        for (int idx = 0; idx < numCols; ++idx) {
            ...
            sb.append(col.getColumnType().getName());
            if (col.getTypeAttributes() != null) {
                sb.append(col.getTypeAttributes().toStringForType(col.getColumnType()));
            }
            sb.append(' ');
...
//                value.reset(); // TODO: Make sure we start at the beginning.
                byte[] data = new byte[value.limit() - value.position()];
                value.get(data);
                if (col.getColumnType() == ColumnType.STRING || col.getColumnType() == ColumnType.VARCHAR) {
                    sb.append('"');
    ...
    public boolean incrementColumn(int index) {
        ColumnSchema column = schema.getColumnByIndex(index);
        ColumnType type = column.getColumnType();
    ...
    private static boolean isCellEqual(PartialRow a, PartialRow b, int index) {
        ...
        Preconditions.checkArgument(a.getSchema().isKey(a.getSchema().getColumnByIndex(index)));
    ...
    private static boolean isCellIncremented(PartialRow lower, PartialRow upper, int index) {
        ...
        Preconditions.checkArgument(lower.getSchema().isKey(lower.getSchema().getColumnByIndex(index)));
    ...
    public PartialRowProto toProtobuf() {
        ...
        PartialRowProto partialRowProto = PartialRowProto.newBuilder()
                ...
                .setRowAlloc(ByteString.copyFrom(rowAlloc))
                .addAllVarLengthData(byteStrings)
                .build();
    ...
    public static PartialRow fromProtobuf(PartialRowProto partialRowProto) {
        ...
        return new PartialRow(Schema.fromProtobuf(partialRowProto.getSchema()),
                byteBuffers,
    ...
    public boolean equals(Object o) {
        ...
        return frozen == that.frozen &&
                ...
                columnsBitSet.equals(that.columnsBitSet) &&
                nullsBitSet.equals(that.nullsBitSet);
    }

    @Override
    public int hashCode() {
        int result = Objects.hash(schema, varLengthData, columnsBitSet, nullsBitSet, frozen);
        result = 31 * result + Arrays.hashCode(rowAlloc);
        return result;
    }
}
