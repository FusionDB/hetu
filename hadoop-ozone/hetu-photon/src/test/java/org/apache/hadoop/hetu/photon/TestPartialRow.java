...
import static org.apache.hadoop.hetu.photon.ClientTestUtil.getPartialRowWithAllTypes;
import static org.apache.hadoop.hetu.photon.ClientTestUtil.getSchemaWithAllTypes;
...
public class TestPartialRow {
    ...
    public void testProto() {
        ...
        HetuPhotonProtos.PartialRowProto partialRowProto = partialRow.toProtobuf();
        assertNotNull(partialRowProto);
        PartialRow partialRowRes = PartialRow.fromProtobuf(partialRowProto);
        assertEquals(partialRow.getSchema().getColumns(), partialRowRes.getSchema().getColumns());
        assertEquals(partialRow.toString(), partialRowRes.toString());
    }
    ...
    public void testAddObject() {
        ...
        for (ColumnSchema col : schema.getColumns()) {
            assertEquals(callGetByName(expected, col.getColumnName(), col.getColumnType()),
                    callGetByName(row, col.getColumnName(), col.getColumnType()));
        }
    }
    ...
    public void testGetUnsetColumn() {
        ...
        for (ColumnSchema columnSchema : schema.getColumns()) {
            ...
            try {
                callGetByName(partialRow, columnSchema.getColumnName(), columnSchema.getColumnType());
                fail("Expected IllegalArgumentException for type: " + columnSchema.getColumnType());
    ...
    public void testGetMissingColumnName() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callGetByName(partialRow, "not-a-column", columnSchema.getColumnType());
                fail("Expected IllegalArgumentException for type: " + columnSchema.getColumnType());
            } catch (IllegalArgumentException ex) {
                // This is the expected exception.
            }
    ...
    public void testGetMissingColumnIndex() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callGetByIndex(partialRow, 999, columnSchema.getColumnType());
                fail("Expected IndexOutOfBoundsException for type: " + columnSchema.getColumnType());
    ...
    public void testGetWrongTypeColumn() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                ...
                if ("null".equals(columnSchema.getColumnName())) {
                    continue;
                }
                callGetByName(partialRow, columnSchema.getColumnName(), getShiftedType(columnSchema.getColumnType()));
                fail("Expected IllegalArgumentException for type: " + columnSchema.getColumnType());
            } catch (IllegalArgumentException ex) {
                // This is the expected exception.
            }
    ...
    public void testAddMissingColumnName() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callAddByName(partialRow, "not-a-column", columnSchema.getColumnType());
                fail("Expected IllegalArgumentException for type: " + columnSchema.getColumnType());
            } catch (IllegalArgumentException ex) {
                // This is the expected exception.
            }
    ...
    public void testAddMissingColumnIndex() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callAddByIndex(partialRow, 999, columnSchema.getColumnType());
                fail("Expected IndexOutOfBoundsException for type: " + columnSchema.getColumnType());
            } catch (IndexOutOfBoundsException ex) {
                // This is the expected exception.
            }
    ...
    public void testAddWrongTypeColumn() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callAddByName(partialRow, columnSchema.getColumnName(), getShiftedType(columnSchema.getColumnType()));
                fail("Expected IllegalArgumentException for type: " + columnSchema.getColumnType());
            } catch (IllegalArgumentException ex) {
                // This is the expected exception.
            }
    ...
    public void testAddToFrozenRow() {
        ...
        for (ColumnSchema columnSchema : partialRow.getSchema().getColumns()) {
            try {
                callAddByName(partialRow, columnSchema.getColumnName(), columnSchema.getColumnType());
                fail("Expected IllegalStateException for type: " + columnSchema.getColumnType());
    ...
    private ColumnType getShiftedType(ColumnType type) {
        int shiftedPosition = (type.ordinal() + 1) % ColumnType.values().length;
        return ColumnType.values()[shiftedPosition];
    }

    private Object callGetByName(PartialRow partialRow, String columnName, ColumnType type) {
    ...
    private Object callGetByIndex(PartialRow partialRow, int columnIndex, ColumnType type) {
    ...
    private void callAddByName(PartialRow partialRow, String columnName, ColumnType type) {
        switch (type) {
            case INT8:
    ...
    private void callAddByIndex(PartialRow partialRow, int columnIndex, ColumnType type) {
        switch (type) {
            case INT8:
