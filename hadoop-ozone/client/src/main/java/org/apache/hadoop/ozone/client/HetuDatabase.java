 ...
 */
public class HetuDatabase extends WithMetadata {
  ...
  public HetuDatabase(ConfigurationSource conf, ClientProtocol proxy,
  ...
  @VisibleForTesting
  protected HetuDatabase(String name, String admin, String owner,
  ...
  public void clearSpaceQuota() throws IOException {
    HetuDatabase hetuDatabase = proxy.getDatabaseDetails(name);
    proxy.setVolumeQuota(name, hetuDatabase.getQuotaInNamespace(), QUOTA_RESET);
    this.quotaInBytes = QUOTA_RESET;
    this.quotaInNamespace = hetuDatabase.getQuotaInNamespace();
  }
  ...
  public void clearNamespaceQuota() throws IOException {
    HetuDatabase hetuDatabase = proxy.getDatabaseDetails(name);
    proxy.setVolumeQuota(name, QUOTA_RESET, hetuDatabase.getQuotaInBytes());
    this.quotaInBytes = hetuDatabase.getQuotaInBytes();
