...
/**
 * Factory class to create HetuClients.
 */
public final class HetuClientFactory {

  private static final Logger LOG = LoggerFactory.getLogger(
      HetuClientFactory.class);

  /**
   * Private constructor, class is not meant to be initialized.
   */
  private HetuClientFactory(){}


  /**
   * Constructs and return an HetuClient with default configuration.
   *
   * @return HetuClient
   *
   * @throws IOException
   */
  public static HetuClient getRpcClient() throws IOException {
    LOG.info("Creating HetuClient with default configuration.");
    return getRpcClient(new OzoneConfiguration());
  }

  /**
   * Returns an HetuClient which will use RPC protocol.
   *
   * @param omHost
   ...
   *
   * @param config
   *        Configuration to be used for HetuClient creation
   *
   * @return HetuClient
   *
   * @throws IOException
   */
  public static HetuClient getRpcClient(String omHost, Integer omRpcPort,
   ...
   * @throws IOException
   */
  public static HetuClient getRpcClient(String omServiceId,
   ...
   *
   * @param config
   *        used for HetuClient creation
   *
   * @return HetuClient
   *
   * @throws IOException
   */
  public static HetuClient getRpcClient(ConfigurationSource config)
      throws IOException {
    Preconditions.checkNotNull(config);
  ...
  /**
   * Creates HetuClient with the given ClientProtocol and Configuration.
   *
   * @param clientProtocol
   *        Protocol to be used by the HetuClient
   *
   * @param config
   *        Configuration to be used for HetuClient creation
   */
  private static HetuClient getRpcClient(ClientProtocol clientProtocol,
                                       ConfigurationSource config) {
    return new HetuClient(config, clientProtocol);
  }

  /**
   * Create HetuClient for token renew/cancel operations.
   * @param conf Configuration to be used for OzoneCient creation
   * @param token ozone token is involved
   * @return
   * @throws IOException
   */
  public static HetuClient getHetuClient(Configuration conf,
    ...
    if (StringUtils.isNotEmpty(omServiceId)) {
      ...
      if (!OmUtils.isServiceIdsDefined(ozoneConf)
        ...
        return HetuClientFactory.getRpcClient(ozoneConf);
      } else if (OmUtils.isOmHAServiceId(ozoneConf, omServiceId)) {
        // HA with matching service id
        return HetuClientFactory.getRpcClient(omServiceId, ozoneConf);
      } else {
