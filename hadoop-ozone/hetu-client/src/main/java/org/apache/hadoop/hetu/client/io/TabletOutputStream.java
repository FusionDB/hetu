 ...
 */
public class TabletOutputStream extends OutputStream {
  ...
  public static final Logger LOG =
      LoggerFactory.getLogger(TabletOutputStream.class);
  ...
  public TabletOutputStream(
    ...
    this.config = config;
    OmTabletInfo info = handler.getTabletInfo();
    blockOutputStreamEntryPool =
        new BlockOutputStreamEntryPool(
            config,
            omClient,
            requestId, factor, type, info,
            unsafeByteBufferConversion,
            xceiverClientManager,
            handler.getId());

    // Retrieve the file encryption tablet info, null if file is not in
    // encrypted bucket.
  ...
  private void handleException(BlockOutputStreamEntry streamEntry,
    ...
    Preconditions.checkArgument(
        offset - blockOutputStreamEntryPool.getTabletLength() == bufferedDataLen);
  ...
  /**
   * Commit the tablet to OM, this will add the blocks as the new tablet blocks.
   *
   * @throws IOException
   */
  @Override
  public void close() throws IOException {
    ...
    try {
      ...
      blockOutputStreamEntryPool.commitTablet(offset);
    } finally {
      blockOutputStreamEntryPool.cleanup();
    }
  }
  ...
  /**
   * Builder class of TabletOutputStream.
   */
  public static class Builder {
    private OpenTabletSession openHandler;
    ...
    public Builder setHandler(OpenTabletSession handler) {
    ...
    public TabletOutputStream build() {
      return new TabletOutputStream(
          clientConfig,
          openHandler,
          xceiverManager,
          omClient,
    ...
    if (closed) {
      throw new IOException(
          ": " + FSExceptionMessages.STREAM_IS_CLOSED + " Key: "
              + blockOutputStreamEntryPool.getTabletName());
    }
  }
}
