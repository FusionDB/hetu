...
/**
 * Base test class for Database request.
 */
@SuppressWarnings("visibilitymodifier")
public class TestOMDatabaseRequest {
  ...
  public void setup() throws Exception {
    ...
    when(ozoneManager.getMaxUserDatabaseCount()).thenReturn(10L);
  ...
  /**
   * Create OMRequest for create database.
   * @param databaseName
   * @param adminName
   * @param ownerName
   * @return OMRequest
   */
  static OMRequest createDatabaseRequest(String databaseName,
      String adminName,
      String ownerName) {
    DatabaseInfo databaseInfo = DatabaseInfo.newBuilder().setName(databaseName)
        .setAdminName(adminName).setOwnerName(ownerName).build();
    CreateDatabaseRequest createDatabaseRequest =
        CreateDatabaseRequest.newBuilder().setDatabaseInfo(databaseInfo).build();
