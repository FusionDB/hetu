...
public class LStoreContainerCheck {

  private static final Logger LOG =
      LoggerFactory.getLogger(LStoreContainerCheck.class);

  private long containerID;
  private LStoreContainerData onDiskContainerData; //loaded from fs/disk
  private ConfigurationSource checkConfig;

  private String metadataPath;

  public LStoreContainerCheck(String metadataPath, ConfigurationSource conf,
                              long containerID) {
  ...
  private void checkContainerFile() throws IOException {
    ...
    LStoreContainerData lStoreData = onDiskContainerData;
    if (!metadataPath.equals(lStoreData.getMetadataPath())) {
      String errStr =
          "Bad metadata path in Containerdata for " + containerID + "Expected ["
              + metadataPath + "] Got [" + lStoreData.getMetadataPath()
              + "]";
      throw new IOException(errStr);
    }
  }
  ...
  private void loadContainerData() throws IOException {
    File containerFile = LStoreContainer
        .getContainerFile(metadataPath, containerID);

    onDiskContainerData = (LStoreContainerData) ContainerDataYaml
