...
import org.apache.hadoop.ozone.tablet.lstore.helpers.LStoreContainerUtil;
...
/**
 * Class to perform LStore Container operations. Any modifications to
 * LStoreContainer object should ideally be done via api exposed in
 * LStoreHandler class.
 */
public class LStoreContainer implements Container<LStoreContainerData> {

  private static final Logger LOG =
          LoggerFactory.getLogger(LStoreContainer.class);

  // Use a non-fair RW lock for better throughput, we may revisit this decision
  // if this causes fairness issues.
  private final ReentrantReadWriteLock lock = new ReentrantReadWriteLock();

  private final LStoreContainerData containerData;
  private ConfigurationSource config;

  public LStoreContainer(LStoreContainerData containerData,
                         ConfigurationSource
      ozoneConfig) {
    Preconditions.checkNotNull(containerData,
            "LStoreContainer cannot be null");
  ...
  public void create(VolumeSet volumeSet, VolumeChoosingPolicy
    ...
    try {
      ...
      LStoreContainerUtil.createContainerMetaData(containerID,
              containerMetaDataPath, chunksPath, dbFile,
              containerData.getSchemaVersion(), config);

      //Set containerData for the LStoreContainer.
  ...
  public void populatePathFields(String scmId,
    ...
    File containerMetaDataPath = LStoreContainerLocationUtil
        .getContainerMetaDataPath(hddsVolumeDir, scmId, containerId);

    File chunksPath = LStoreContainerLocationUtil.getChunksLocationPath(
        hddsVolumeDir, scmId, containerId);
    File dbFile = LStoreContainerLocationUtil.getContainerDBFile(
  ...
  private void writeToContainerFile(File containerFile, boolean isCreate)
    ...
    try {
      ...
      ContainerDataYaml.createContainerFile(
          ContainerType.LStoreContainer, containerData, tempContainerFile);
  ...
  public void delete() throws StorageContainerException {
    long containerId = containerData.getContainerID();
    try {
      LStoreContainerUtil.removeContainer(containerData, config);
    } catch (StorageContainerException ex) {
      throw ex;
    } catch (IOException ex) {
  ...
  @Override
  public LStoreContainerData getContainerData()  {
    return containerData;
  }

  @Override
  public ContainerDataProto.State getContainerState() {
    return containerData.getState();
  }

  @Override
  public ContainerType getContainerType() {
    return ContainerType.LStoreContainer;
  }
  ...
  public void importContainerData(InputStream input,
      ContainerPacker<LStoreContainerData> packer) throws IOException {
    writeLock();
    try {
      ...
      LStoreContainerData originalContainerData =
          (LStoreContainerData) ContainerDataYaml
      ...
      LStoreContainerUtil.parseKVContainerData(containerData, config);
  ...
  public void exportContainerData(OutputStream destination,
    ...
    try {
      ...
      if (!(state == ContainerDataProto.State.CLOSED ||
  ...
  public void writeLock() {
    // TODO: The lock for LStoreContainer object should not be exposed
    // publicly.
  ...
  /**
   * Returns LStoreContainerReport for the LStoreContainer.
   */
  @Override
  ...
  public boolean scanMetaData() {
    long containerId = containerData.getContainerID();
    LStoreContainerCheck checker =
        new LStoreContainerCheck(containerData.getMetadataPath(), config,
            containerId);
