...
public class FilePerChunkStrategy implements ChunkManager {
  ...
  public void writeChunk(Container container, BlockID blockID, ChunkInfo info,
    ...
    try {
      LStoreContainer lStoreContainer = (LStoreContainer) container;
      LStoreContainerData containerData = lStoreContainer.getContainerData();
      HddsVolume volume = containerData.getVolume();
      VolumeIOStats volumeIOStats = volume.getVolumeIOStats();

      File chunkFile = getChunkFile(lStoreContainer, blockID, info);
  ...
  public ChunkBuffer readChunk(Container container, BlockID blockID,
    ...
    // of the chunk file.
    File finalChunkFile = getChunkFile(lStoreContainer, blockID, info);
    ...
    if (info.getOffset() != 0) {
      try {
        BlockData blockData = blockManager.getBlock(lStoreContainer, blockID);
  ...
  private static File getChunkFile(LStoreContainer container, BlockID blockID,
