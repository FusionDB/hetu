 ...
 */
public class FilePerSegmentStrategy implements ChunkManager {

  private static final Logger LOG =
      LoggerFactory.getLogger(FilePerSegmentStrategy.class);

  private final boolean doSyncWrite;
  private final OpenFiles files = new OpenFiles();
  private final long defaultReadBufferCapacity;

  public FilePerSegmentStrategy(boolean sync, BlockManager manager) {
    doSyncWrite = sync;
  ...
  private static void checkLayoutVersion(Container container) {
    Preconditions.checkArgument(
        container.getContainerData().getLayOutVersion() == FILE_PER_SEGMENT);
  }
  ...
  @Override
  public void finishWriteChunks(LStoreContainer container,
  ...
  private File getChunkFile(Container container, BlockID blockID,
    ...
    return FILE_PER_SEGMENT.getChunkFile(container.getContainerData(), blockID,
        info);
  }
