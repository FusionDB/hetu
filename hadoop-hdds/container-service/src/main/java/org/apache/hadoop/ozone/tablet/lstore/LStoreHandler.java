...
package org.apache.hadoop.ozone.tablet.lstore;
...
import org.apache.hadoop.ozone.tablet.lstore.impl.ChunkManagerFactory;
...
/**
 * Handler for LStore Container type.
 */
public class LStoreHandler extends Handler {
  ...
  public LStoreHandler(ConfigurationSource config, String datanodeId,
  ...
  public ContainerCommandResponseProto handle(
    ...
    return LStoreHandler
        .dispatchRequest(this, request, (LStoreContainer) container,
            dispatcherContext);
  }

  @VisibleForTesting
  static ContainerCommandResponseProto dispatchRequest(LStoreHandler handler,
                                                       ContainerCommandRequestProto request, LStoreContainer lStoreContainer,
                                                       DispatcherContext dispatcherContext) {
    Type cmdType = request.getCmdType();

    switch(cmdType) {
    case CreateContainer:
      return handler.handleCreateContainer(request, lStoreContainer);
    case ReadContainer:
      return handler.handleReadContainer(request, lStoreContainer);
    case UpdateContainer:
      return handler.handleUpdateContainer(request, lStoreContainer);
    case DeleteContainer:
      return handler.handleDeleteContainer(request, lStoreContainer);
    case ListContainer:
      return handler.handleUnsupportedOp(request);
    case CloseContainer:
      return handler.handleCloseContainer(request, lStoreContainer);
    case PutBlock:
      return handler.handlePutBlock(request, lStoreContainer, dispatcherContext);
    case GetBlock:
      return handler.handleGetBlock(request, lStoreContainer);
    case DeleteBlock:
      return handler.handleDeleteBlock(request, lStoreContainer);
    case ListBlock:
      return handler.handleUnsupportedOp(request);
    case ReadChunk:
      return handler.handleReadChunk(request, lStoreContainer, dispatcherContext);
    case DeleteChunk:
      return handler.handleDeleteChunk(request, lStoreContainer);
    case WriteChunk:
      return handler.handleWriteChunk(request, lStoreContainer, dispatcherContext);
    case ListChunk:
      return handler.handleUnsupportedOp(request);
    case CompactChunk:
      return handler.handleUnsupportedOp(request);
    case PutSmallFile:
      return handler
          .handlePutSmallFile(request, lStoreContainer, dispatcherContext);
    case GetSmallFile:
      return handler.handleGetSmallFile(request, lStoreContainer);
    case GetCommittedBlockLength:
      return handler.handleGetCommittedBlockLength(request, lStoreContainer);
    default:
      return null;
    }
  }
  ...
  private void populateContainerPathFields(LStoreContainer container)
      throws IOException {
    volumeSet.readLock();
    try {
  ...
  ContainerCommandResponseProto handleReadContainer(
    ...
    try {
      checkContainerIsHealthy(lStoreContainer);
    } catch (StorageContainerException sce) {
      return ContainerUtils.logAndReturnError(LOG, sce, request);
    }

    LStoreContainerData containerData = lStoreContainer.getContainerData();
  ...
  ContainerCommandResponseProto handleUpdateContainer(
    ...
    try {
      if (!metadata.isEmpty()) {
        lStoreContainer.update(metadata, forceUpdate);
      }
    } catch (StorageContainerException ex) {
      return ContainerUtils.logAndReturnError(LOG, ex, request);
    }
  ...
  ContainerCommandResponseProto handleDeleteContainer(
    ...
    try {
      deleteInternal(lStoreContainer, forceDelete);
    } catch (StorageContainerException ex) {
      return ContainerUtils.logAndReturnError(LOG, ex, request);
    }
    return getSuccessResponse(request);
  ...
  ContainerCommandResponseProto handleCloseContainer(
    ...
    try {
      markContainerForClose(lStoreContainer);
      closeContainer(lStoreContainer);
    } catch (StorageContainerException ex) {
      return ContainerUtils.logAndReturnError(LOG, ex, request);
  ...
  ContainerCommandResponseProto handlePutBlock(
    ...
    try {
      checkContainerOpen(lStoreContainer);
      ...
      if (!request.getPutBlock().hasEof() || request.getPutBlock().getEof()) {
        chunkManager.finishWriteChunks(lStoreContainer, blockData);
        incrKeyCount = true;
      }
      ...
      blockManager.putBlock(lStoreContainer, blockData, incrKeyCount);
  ...
  ContainerCommandResponseProto handleGetBlock(
    ...
    try {
      ...
      responseData = blockManager.getBlock(lStoreContainer, blockID)
          .getProtoBufMessage();
  ...
  ContainerCommandResponseProto handleGetCommittedBlockLength(
    ...
    try {
      ...
      blockLength = blockManager.getCommittedBlockLength(lStoreContainer, blockID);
    } catch (StorageContainerException ex) {
      return ContainerUtils.logAndReturnError(LOG, ex, request);
    } catch (IOException ex) {
      return ContainerUtils.logAndReturnError(LOG,
   ...
   * @param lStoreContainer
   */
  @VisibleForTesting
  void checkContainerIsHealthy(LStoreContainer lStoreContainer)
      throws StorageContainerException {
    lStoreContainer.readLock();
    try {
      if (lStoreContainer.getContainerData().getState() == State.UNHEALTHY) {
        throw new StorageContainerException(
            "The container(" + lStoreContainer.getContainerData().getContainerID() +
            ") replica is unhealthy.", CONTAINER_UNHEALTHY);
      }
    } finally {
      lStoreContainer.readUnlock();
    }
  }
  ...
  ContainerCommandResponseProto handleDeleteChunk(
    ...
    try {
      ...
      chunkManager.deleteChunk(lStoreContainer, blockID, chunkInfo);
    } catch (StorageContainerException ex) {
      return ContainerUtils.logAndReturnError(LOG, ex, request);
    } catch (IOException ex) {
      return ContainerUtils.logAndReturnError(LOG,
  ...
  ContainerCommandResponseProto handleGetSmallFile(
    ...
    try {
      BlockID blockID = BlockID.getFromProtobuf(getSmallFileReq.getBlock()
          .getBlockID());
      BlockData responseData = blockManager.getBlock(lStoreContainer, blockID);
      ...
      for (ContainerProtos.ChunkInfo chunk : responseData.getChunks()) {
        ...
        ChunkBuffer data = chunkManager.readChunk(lStoreContainer, blockID,
   ...
   * @throws StorageContainerException
   */
  private void checkContainerOpen(LStoreContainer lStoreContainer)
      throws StorageContainerException {

    final State containerState = lStoreContainer.getContainerState();
...
//        new LStoreContainerData(originalContainerData);
//
//    LStoreContainer container = new LStoreContainer(containerData,
//        conf);
//
...
//    return container;
  ...
  public void exportContainer(final Container container,
      ...
      final TarContainerPacker packer)
      throws IOException{
//    final LStoreContainer kvc = (LStoreContainer) container;
